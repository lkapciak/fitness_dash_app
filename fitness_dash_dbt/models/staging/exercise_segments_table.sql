...
where not exists (
    ...
    where existing.parent_key = src.parent_key
      and existing.segment_start_time = {{ cast_unixepoch_to_datetime('src.segment_start_time') }}
      and existing.segment_end_time = {{ cast_unixepoch_to_datetime('src.segment_end_time') }}
)
